...
const createReview = async (req, res, next) => {
  try {
    ...
    const { rate, content, to } = req.body;
    ...
    const validReviewInfo = CreateReviewService({ content, rate });

    // Create review in our database
    await Review.create({ ...validReviewInfo, from, to });

    const reviews = await Review.find({ to });
    const rates = reviews.map(review => review.rate);

    const calculatedRate = CalculateFreelancerRateService(rates);

    if(calculatedRate < 2 && reviews.length > 10){
...
const updateReview = async (req, res, next) => {
  try {
    ...
    console.log(userId, isReviewExist.from.toString());
    ...
    console.log(validReviewInfo);

    if (result instanceof AppError) throw result;

    // Update review in our database
    const targetReview = await Review.findByIdAndUpdate(id, { ...validReviewInfo });
