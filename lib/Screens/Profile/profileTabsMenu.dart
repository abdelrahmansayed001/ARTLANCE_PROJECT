...
class _profileTabsMenuState extends State<profileTabsMenu>
  ...
  postReview() async {
    ...
    if (rateText.text.isEmpty) {
      var snackBar = const SnackBar(
        content: Text('Your rate is empty!'),
        duration: Duration(seconds: 1),
      );
      ScaffoldMessenger.of(context).showSnackBar(snackBar);
      return;
    }
    if (rateText.text.isNotEmpty) {
      var num = int.parse(rateText.text);
      if(num < 1 || num > 5){
        var snackBar = const SnackBar(
          content: Text('Your rate should range from 1 to 5'),
          duration: Duration(seconds: 1),
        );
        ScaffoldMessenger.of(context).showSnackBar(snackBar);
        return;
                        ...
                        child: Column(
                          children: [
                            ...
                            _buildRow(const Icon(Icons.money_off_sharp),
                                "Description: ", "${widget.description}", 1, false),
                            const SizedBox(
                              height: 5,
                            ),
                            const Divider(
                            ...
                            _buildRow(const Icon(Icons.money_off_sharp),
                                "Hourly Rate: ", "${widget.hourlyRate} EGP", 2, true),
                            const Divider(
                              color: Colors.black,
                            ),
                            _buildRow(
  ...
  _buildRow(Icon icon, text1, text2, int type, bool isRow) {
    return SizedBox(
